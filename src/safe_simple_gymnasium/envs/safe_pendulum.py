...
class SafePendulumEnv(gym.Env):
    ...
    *-(pi<sup>2</sup> + 0.1 * 8<sup>2</sup> + 0.001 * 2<sup>2</sup>) = -16.2736044*,
    ...
    A cost of `$1$` is incurred if the angle of the pendulum is between [pi/4, pi/2]
    ...
    * v0: Initial versions release (1.0.0)
    ...
    def step(self, u):
        ...
        info = {'cost': is_unsafe(self.state[0])}
...
def is_unsafe(th):
    ...
    return np.pi / 4 <= norm_th <= np.pi / 2